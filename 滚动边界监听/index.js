(function (win) {
    ...
    ScrollEdgeWatcher.prototype._initParams = function (options) {
        ...
        opts.onBeforeDestroy = (options.onBeforeDestroy ?? noop).bind(this.el, this.el)
    ...
    ScrollEdgeWatcher.prototype._getParams = function () {
        ...
        const params = {
            ...
            isReachTop: !this._reachEdgeStatus.isReachTop && newReachEdgeStatus.isReachTop,
            // 上次没有到达最右侧但这次到达最右侧了。如果不进行比对，则会重复触发到达最右侧逻辑，并且也会误判到达最右侧方向问题，导致向左滚动也会到触发到达最右侧的逻辑
            isReachRight: !this._reachEdgeStatus.isReachRight && newReachEdgeStatus.isReachRight,
            // 上次没有触底但这次触底了。如果不进行比对，则会重复触发触底逻辑，并且也会误判触顶方向问题，导致向上滚动也会触底逻辑
            isReachBottom: !this._reachEdgeStatus.isReachBottom && newReachEdgeStatus.isReachBottom,
            // 上次没有到达最左侧但这次到达最左侧了。如果不进行比对，则会重复触发到达最左侧逻辑，并且也会误判到达最做侧方向问题，导致向右滚动也会到触发到达最左侧的逻辑
            isReachLeft: !this._reachEdgeStatus.isReachLeft && newReachEdgeStatus.isReachLeft,
        }
    ...
    ScrollEdgeWatcher.prototype._getUsableOffset = function () {
        ...
        return {
            ...
            top: this.el.scrollHeight - this.el.clientHeight < offsetTop ? 0 : offsetTop,
            // 当可滚动的距离 < 设定右侧偏移距离，则设为 0，因为永远无法到达该设定的右侧偏移值
            right: this.el.scrollWidth - this.el.clientWidth < offsetRight ? 0 : offsetRight,
            // 当可滚动的距离 < 设定底部偏移距离，则设为 0，因为永远无法到达该设定的底部偏移值
            bottom: this.el.scrollHeight - this.el.clientHeight < offsetBottom ? 0 : offsetBottom,
            // 当可滚动的距离 < 设定左侧偏移距离，则设为 0，因为永远无法到达该设定的左侧偏移值
            left: this.el.scrollWidth - this.el.clientWidth < offsetLeft ? 0 : offsetLeft
        }
    }
    ...
    ScrollEdgeWatcher.prototype._getReachEdgeStatus = function () {
        ...
        return {
            ...
            // 判断是否到达最右侧
            isReachRight: this._scroll.scrollLeft + this.el.clientWidth >= this.el.scrollWidth - right,
            // 判断是否触底.
            isReachBottom: this._scroll.scrollTop + this.el.clientHeight >= this.el.scrollHeight - bottom,
            // 判断是否到达最左侧
